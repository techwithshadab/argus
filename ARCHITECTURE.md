...
On every start the replay task applies every `data/sql/*.sql` except `000_*` (idempotent, so Aurora and a fresh local volume converge), loads reference data, encrypts personal data with the data key, builds the ownership network, ensures the daily partitions, bulk-loads the scenario, publishes the area and ground truth to `scenario_meta`, then streams positions on a loop so the floor never goes quiet. Live mode subscribes AISStream to that bounding box together with every region named by `WATCH_AREAS` from `data/areas.yaml`, in one subscription, and injects nothing (ADR-0021). Out-of-range coordinates and null-island reports are dropped before the insert, and the task publishes its own heartbeat so a silent feed can page.
...
A tool-less model call over the merged findings produces the VOI report: headline, priority, confidence, timeline, indicators and counter-indicators, ownership and sanctions, recommended actions, collection plan, evidence, gaps, caveats. The policy check accepts only allowed action patterns and only indicators whose source a tool produced. Problems are split into hard and soft (ADR-0020): a hard problem is retried once with the violations listed and fails the investigation closed if it survives, while the one soft problem, a report stating neither counter-indicators nor information gaps, is accepted after the retry with a caveat saying so. A guardrail block buys a third attempt with a plainer correction list, because the prompt-attack filter reads the correction text too. A failed Investigator branch also caps the report's confidence and priority before the check runs.

### 6. Review and approval (`services/api/main.py`, `services/ui/index.html`)

Alerts and reports are reviewed (accept or reject, Shift-key shortcuts so stray typing cannot decide anything); tasking requests are approved or rejected. Every decision records who and when and appends an audit event. On AWS the officer's identity is the load balancer's Cognito sign-in, verified from the signed `x-amzn-oidc-data` token on every non-public route (`services/api/officerauth.py`, ADR-0018); a request header stands in only for local development. A decision is also one-way: reviewing an alert or a report a second time is refused rather than allowed to overwrite the first officer's verdict.
