...
| POST | `/alerts/{id}/review` | `{decision: accepted\|rejected, note?}`; records reviewer and time |
...
| POST | `/investigations/{id}/complete` *agent* | `{report, trace_id, manifest}`; snapshots evidence, marks alerts investigated |
| POST | `/investigations/{id}/fail` *agent* | `{error, trace_id}` |
| POST | `/investigations/{id}/review` | `{decision: accepted\|rejected}` on a completed report |
...
| GET | `/evidence/{id}` | One snapshot with payload |
...
Each specialist serves the A2A protocol (JSON-RPC `message/send`) on port 9000 with an agent card at `/.well-known/agent.json`, `/ping` for health and `/provenance` for its model, tier and prompt hash. The orchestrator serves the AgentCore HTTP contract (`POST /invocations`, `/ping`) on 8080 with payloads `{"mmsi", "trigger", "investigation_id", "alert"}` or `{"action": "sweep", "hours"}`.
